...
public interface BookingTableRepository extends JpaRepository<BookingTable, Long> {
    ...
    boolean existsByTableAndStartTimeBetween(Table table, LocalDateTime start, LocalDateTime end);
    boolean existsByTableAndStartTimeLessThanAndEndTimeGreaterThan(Table table, LocalDateTime endTime, LocalDateTime startTime);
    boolean existsByTableAndEndTimeAfterAndStartTimeBefore(Table table, LocalDateTime startTime, LocalDateTime endTime);
    ...
    boolean existsByTableAndStartTimeBeforeAndEndTimeAfter(Table table, LocalDateTime startTime, LocalDateTime endTime);
    boolean existsByTableAndStartTimeBetweenAndIdNot(Table table, LocalDateTime start, LocalDateTime end, Long id);
    boolean existsByTableAndEndTimeAfterAndStartTimeBeforeAndIdNot(Table table, LocalDateTime start, LocalDateTime end, Long id);
    boolean existsByTableAndStartTimeBeforeAndEndTimeAfterAndIdNot(Table table, LocalDateTime start, LocalDateTime end, Long id);
    List<BookingTable> findByStartTimeBeforeAndUpdatedFalse(LocalDateTime currentTime);
    List<BookingTable> findByEndTimeBeforeAndUpdatedTrue(LocalDateTime currentTime);
